...
- [ ] Every output page has a recorded rendered-verification result, whether
      or not it was already classified as searchable text.
- [ ] A planted sensitive value that exists only in an embedded raster image
...
- [ ] Any unresolved rendered-OCR match blocks `AUTOMATED_PASS`; a
      resolved/suppressed one does not.
- [ ] The hard-coded `raster_ocr_from_sanitized_images_only` field no longer
      appears anywhere in the report.
- [ ] A fresh run of the 43-page MLK reference document contains no
